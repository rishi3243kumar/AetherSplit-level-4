...
/**
 * Creates a private bill (AetherSplit) by calling the BillRegistry contract.
...
export const createPrivateBillOnChain = async (
  ...
  // In a fully deployed environment, this would call executeContractCall to `create_bill` on the new AetherSplit registry.
  ...
  try {
    const existing = localStorage.getItem('aethersplit_simulated_events');
    const list = existing ? JSON.parse(existing) : [];
    list.push(simulatedEvent);
    localStorage.setItem('aethersplit_simulated_events', JSON.stringify(list));
    
    // Dispatch custom event to update ActivityFeed in real time
    window.dispatchEvent(new CustomEvent('aethersplit_new_event', { detail: simulatedEvent }));
  } catch (e) {
